...
window.onload = function () {
  ...
  // 3. | Font | Bigger | Smaller | Bold | Italic | Underline | Strikethrough | Superscript | Subscript | Text Color | Highlight |
  ...
  // 4. | Table | Image | Hyperlink | Separator | Watermark | Code Block | Separator | Control | Checkbox | LaTeX | Date Selector
  ...
  // Column/Section break control
  const columnDom =
    document.querySelector<HTMLDivElement>('.menu-item__column')!
  const columnOptionDom = columnDom.querySelector<HTMLDivElement>('.options')!
  columnDom.onclick = function () {
    console.log('column')
    columnOptionDom.classList.toggle('visible')
  }
  columnOptionDom.onmousedown = function (evt) {
    columnOptionDom.classList.toggle('visible')
    const li = evt.target as HTMLLIElement
    const columns = Number(li.dataset.columns || 1)
    const breakType =
      (li.dataset.break as 'nextPage' | 'continuous') || 'continuous'
    if (!columns || columns < 1) return
    instance.command.executeInsertSectionBreak({ columns, breakType })
  }
        ...
        new Dialog({
          ...
          data: [
            ...
            },
            {
              ...
              required: true,
              height: 100,
              placeholder: `Please enter value sets JSON, e.g.:\n[{\n"value":"Yes",\n"code":"98175"\n}]`
            }
          ],
          onConfirm: payload => {
            const placeholder = payload.find(
          ...
          data: [
            {
              ...
              name: 'code',
              placeholder: 'Please enter default values, separated by commas'
            },
            {
              type: 'textarea',
              label: 'Value Sets',
      ...
      data: [
        {
          ...
          height: 100,
          name: 'value',
          placeholder: 'Please enter LaTeX'
        }
      ],
      onConfirm: payload => {
        const value = payload.find(p => p.name === 'value')?.value
    ...
    new Dialog({
      ...
      data: [
        ...
        },
        {
          ...
          label: 'Width',
          name: 'width',
          placeholder: 'Please enter width (defaults to page inner width)'
        },
        {
          type: 'number',
          label: 'Height',
        ...
        },
        {
          ...
          name: 'srcdoc',
          required: false,
          placeholder: 'Please enter HTML (only effective for Website type)'
        }
      ],
      onConfirm: payload => {
  ...
  // 6. Catalog toggle | Page mode | Page scale | Paper size | Paper direction | Margins | Fullscreen | Settings
