...
export class Position {
  ...
  public computePositionList() {
    ...
    for (let i = 0; i < pageRowList.length; i++) {
      ...
      // Segment by column (columnStartX, columnInnerWidth)
      let runStart = 0
      // Track per-page vertical baseline and per-column offsets
      let baseY = 0
      const yByCol = new Map<string, number>()
      const flushRun = (runEnd: number) => {
        if (runEnd <= runStart) return
        const run = rowList.slice(runStart, runEnd)
        const first = run[0]
        const innerWidth = first.columnInnerWidth || this.draw.getInnerWidth()
        const startIndex = first.startIndex
        const startX = baseStartX + (first.columnStartX || 0)
        // Determine this run's vertical start (inside content area)
        let runStartYOffset = 0
        ...
        if (isColumnRun) {
          ...
          if (!yByCol.has(colKey)) {
            // First time seeing this column on this page: align to current baseY
            yByCol.set(colKey, baseY)
          }
          runStartYOffset = yByCol.get(colKey) || 0
        } else {
          // Before laying out normal content, if we were in a column block, finalize baseY to tallest column
          if (yByCol.size) {
            let maxY = baseY
        ...
        this.computePageRowPosition({
          ...
          startIndex,
          startX,
          startY: startY + runStartYOffset,
          innerWidth
        })
        startRowIndex += run.length
        // Advance offsets: accumulate this run's height into column or base
        let consumedY = 0
      ...
      for (let r = 1; r < rowList.length; r++) {
        ...
        const sectionChanged = (prev.sectionId || '') !== (cur.sectionId || '')
        if (prevKeyX !== curKeyX || prevW !== curW || sectionChanged) {
          // Determine if previous run was a column run
          const runFirst = rowList[runStart]
          ...
          // flush previous run [runStart, r)
          flushRun(r)
          // If we are switching between distinct column blocks (e.g., section change),
          // finalize baseY to the tallest column from the previous block to avoid overlap.
      ...
      // flush last run
      flushRun(rowList.length)
      // End of page: clear any pending column offsets
      if (yByCol.size) {
        let maxY = baseY
        for (const val of yByCol.values()) {
          if (val > maxY) maxY = val
    ...
    for (let j = 0; j < positionList.length; j++) {
      ...
      if (
        ...
        if (elementList[index].value !== ZERO) {
          ...
          // In multi-column layouts, don't adjust coordinates
          // Character coordinates are already in page coordinate system
          const adjustedX = x

          if (adjustedX < leftTop[0] + valueWidth / 2) {
          ...
          // Debug output for multi-column index calculation
          const pageRowList = this.draw.getPageRowList()
          if (!isTable && isMainActive && pageRowList[positionNo]) {
            const rowList = pageRowList[positionNo]
            const position = positionList[j]
            const row = rowList[position.rowIndex]

            if (row && row.columnStartX !== undefined) {
              console.log('Multi-column position debug:', {
                clickX: x,
                adjustedX: adjustedX,
                elementValue: elementList[index].value,
                elementIndex: index,
                positionIndex: j,
                curPositionIndex,
                hitLineStartIndex,
                rowIndex: position.rowIndex,
                columnStartX: row.columnStartX,
                leftTopX: leftTop[0],
                rightTopX: rightTop[0],
                valueWidth: valueWidth,
                charCenter: leftTop[0] + valueWidth / 2,
                positionValue: position.value,
                // Additional debugging for character type
                isZero: elementList[index].value === ZERO,
                isPunctuation: [
                  '.',
                  '。',
                  ',',
                  ...
                  '!',
                  '！'
                ].includes(elementList[index].value),
                prevElement: index > 0 ? elementList[index - 1].value : 'N/A',
                nextElement:
                  index < elementList.length - 1
                    ? elementList[index + 1].value
                    : 'N/A',
                // Debug the actual dot position if it exists
                dotElementIndex: elementList.findIndex(
                  (el, idx) => idx > index && el.value === '.'
                ),
                dotPosition:
                  elementList.findIndex(
                    ...
                    ? (() => {
                        const dotIdx = elementList.findIndex(
                          (el, idx) => idx > index && el.value === '.'
                        )
                        const dotPos = positionList[dotIdx]
                        return dotPos
                          ? {
                              index: dotIdx,
                              leftTop: dotPos.coordinate.leftTop[0],
                              rightTop: dotPos.coordinate.rightTop[0]
                            }
                          : null
                      })()
                    : null
        ...
        // Multi-column index correction - simple fix for consistent off-by-1 error
        let finalPositionIndex = curPositionIndex
        const pageRowList = this.draw.getPageRowList()
        if (!isTable && isMainActive && pageRowList[positionNo]) {
          const rowList = pageRowList[positionNo]
          const position = positionList[j]
          ...
          if (row && row.columnStartX !== undefined) {
            // Simple fix: In multi-column layouts, position detection is consistently off
            // Add 2 to the index to correct for the systematic error
            finalPositionIndex = curPositionIndex + 2
