...
export class CommandAdapt {
  ...
  public insertSectionBreak(
    ...
    // If there's no selection (collapsed), keep original behavior: insert a single section break
    const { startIndex, endIndex } = this.range.getRange()
    const isCollapsed = startIndex === endIndex
    if (isCollapsed) {
      // Delegate to Draw API which creates SECTION_BREAK element and inserts at caret
      this.draw.insertSectionBreak(payload, options)
      return
    }

    // Non-collapsed selection: wrap selection in a section with desired columns,
    // starting at the first character of the paragraph containing the selection start (anchor),
    // and insert a restoring section break after the selection so columns don't leak.
    const elementList = this.draw.getElementList()
    if (!elementList.length) return

    // Determine the effective section config at the point immediately after the selection to restore afterward
    ...
    // Scan from start to the end of selection for the last SECTION_BREAK encountered
    // This preserves any existing section config that would have applied after the selection.
    for (let i = 0; i <= endIndex; i++) {
      const el = elementList[i]
      if (el && el.type === ElementType.SECTION_BREAK) {
        const cfg = (el as any).config || {}
        restoreConfig = {
          columns: Math.max(1, Number(cfg.columns) || 1),
          gap: Number(cfg.gap ?? defaultGap),
          rule: !!cfg.rule,
          balance: !!cfg.balance
        }
      }
    ...
    // Build start and end section break elements
    const startBreak = {
    ...
    const endBreak = {
      ...
      // Always continue after the selection; don't force a new page
      breakType: 'continuous'
    } as any

    // Compute insertion indices on the original list to avoid index shift bugs
    // Start insert: at the first character of the paragraph containing selection start
    let startInsertPos = startIndex
    while (startInsertPos > 0) {
      const prev = elementList[startInsertPos - 1]
      if (prev && prev.value === ZERO) break
      startInsertPos--
    }
    // End insert: after the end of the paragraph containing selection end
    let endInsertPos = endIndex + 1
    while (endInsertPos < elementList.length) {
      const cur = elementList[endInsertPos]
      if (cur && cur.value === ZERO) {
        endInsertPos++ // insert AFTER the ZERO
        break
      }
      endInsertPos++
    }
    // Insert end break first (higher index), then start break
    this.draw.spliceElementList(elementList, endInsertPos, 0, [endBreak])
    this.draw.spliceElementList(elementList, startInsertPos, 0, [startBreak])

    // Collapse cursor to the first visible element AFTER the closing break to ensure typing inserts at the pointer
    let caretIndex = endInsertPos + 1
    while (caretIndex < elementList.length) {
      const el = elementList[caretIndex]
      // Skip invisible/non-rendered separators like SECTION_BREAK
      if (el && el.type !== ElementType.SECTION_BREAK) break
      caretIndex++
    }
    if (caretIndex >= elementList.length) {
      // fallback: move to last visible element before the break
      caretIndex = Math.max(0, endInsertPos - 1)
      while (caretIndex >= 0) {
        const el = elementList[caretIndex]
        if (el && el.type !== ElementType.SECTION_BREAK) break
        caretIndex--
    ...
    this.range.setRange(caretIndex, caretIndex)
    this.draw.render({ curIndex: caretIndex })
  }
