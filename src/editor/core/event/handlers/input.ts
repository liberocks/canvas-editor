...
export function input(data: string, host: CanvasEvent) {
  ...
  if (control.getActiveControl() && control.getIsRangeWithinControl()) {
    ...
    // Multi-column cursor fix: adjust insertion point for multi-column layouts
    const pageRowList = draw.getPageRowList()
    const currentPageNo = draw.getPageNo()
    let adjustedStart = start

    if (pageRowList[currentPageNo]) {
      const rowList = pageRowList[currentPageNo]
      const positionList = draw.getPosition().getPositionList()

      // Check if we're in a multi-column context
      if (positionList[startIndex]) {
        const position = positionList[startIndex]
    ...
    if (startIndex !== endIndex) {
      draw.spliceElementList(elementList, adjustedStart, endIndex - startIndex)
    }
    formatElementContext(elementList, inputData, startIndex, {
      editorOptions: draw.getOptions()
    })
    draw.spliceElementList(elementList, adjustedStart, 0, inputData)
