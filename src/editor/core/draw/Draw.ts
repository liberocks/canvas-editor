...
export class Draw {
  ...
  constructor(
    ...
    // In print mode, initialize print data first
  ...
  // ---- Sections & Columns (multi-column) ----
  private _splitIntoSections(elementList: IElement[]) {
    // Default single-column section
    const defaultGap = this.options.column?.defaultGap ?? 24
    type SectionInfo = {
      id: string
      config: {
        columns: number
      ...
      breakType: 'continuous' | 'nextPage'
      elements: IElement[]
      baseIndex: number // index in main elementList of the first element in this section
    }
    const sections: SectionInfo[] = []
    let current: SectionInfo = {
      id: 'section-0',
      config: { columns: 1, gap: defaultGap },
      breakType: 'continuous',
      elements: [],
      baseIndex: 0
    }
    let globalIndex = 0 // counts non-section-break elements in main list
    let sectionIndex = 1
    for (let i = 0; i < elementList.length; i++) {
      const el = elementList[i]
      if (el.type === ElementType.SECTION_BREAK) {
        // finalize current
        sections.push(current)
        const sectionEl = el as any
        current = {
          id: sectionEl.sectionId || `section-${sectionIndex++}`,
          config: {
            columns: Math.max(1, Number(sectionEl.config?.columns) || 1),
            gap: Number(sectionEl.config?.gap ?? defaultGap),
            rule: !!sectionEl.config?.rule,
            balance: !!sectionEl.config?.balance
          },
          breakType:
            sectionEl.breakType === 'nextPage' ? 'nextPage' : 'continuous',
          elements: [],
  ...
  private _computeSectionColumns(
    pageInnerWidth: number,
    columns: number,
    gap: number
  ) {
    const realColumns = Math.max(1, Math.floor(columns))
    const realGap = Math.max(0, gap)
    const totalGap = (realColumns - 1) * realGap
    const columnWidth =
      Math.max(0, Math.floor(pageInnerWidth - totalGap)) / realColumns
    const columnStartXList: number[] = []
    for (let i = 0; i < realColumns; i++) {
      columnStartXList.push(i * (columnWidth + realGap))
    }
    return { columnWidth, columnStartXList, gap: realGap, columns: realColumns }
  }

  private _distributeRowsIntoColumns(
    rows: IRow[],
    sectionInfo: {
      columnWidth: number
      columnStartXList: number[]
      columns: number
    },
    breakType: 'continuous' | 'nextPage',
    sectionId: string,
    initialY = 0,
    firstPageCapHeight?: number
  ): IRow[][] {
    const pages: IRow[][] = []
    ...
    const pageTopY = margins[0] + headerExtraHeight
    const pageBottomY = height - (margins[2] + footerExtraHeight)
    const pageContentHeight = pageBottomY - pageTopY
    let currentPageRows: IRow[] = []
    let currentColumn = 0
    let pageInitialY = Math.max(0, initialY)
    let currentY = pageInitialY
    let isFirstPageOfSection = true
    const startNewPage = () => {
      if (currentPageRows.length) pages.push(currentPageRows)
      currentPageRows = []
      currentColumn = 0
      // New page starts at top content area
      pageInitialY = 0
      currentY = 0
      isFirstPageOfSection = false
    }
    if (breakType === 'nextPage' && pages.length === 0) {
      // force new page at section start, ignore any initialY
      startNewPage()
    }
    // Ensure at least one page
    if (!pages.length) {
      // no-op; we will push at end; but keep collector ready
    }
    for (let i = 0; i < rows.length; i++) {
      ...
      const pageMaxHeight =
        ...
        breakType === 'continuous' &&
        sectionInfo.columns > 1
          ? (firstPageCapHeight ?? pageContentHeight - pageInitialY)
          : pageContentHeight
      const fits = currentY - pageInitialY + rowHeight <= pageMaxHeight
      if (!fits) {
        // advance column or page
        if (currentColumn < sectionInfo.columns - 1) {
          currentColumn += 1
          // Next column on the same page starts from the block's baseline
          currentY = pageInitialY
        } else {
          // next page
          startNewPage()
        }
      }
      // Place row (may be after advancing)
      row.sectionId = sectionId
      row.columnIndex = currentColumn
      row.columnStartX = sectionInfo.columnStartXList[currentColumn]
      row.columnInnerWidth = sectionInfo.columnWidth
  ...
  private _computePageListWithSections(): IRow[][] {
    const sections = this._splitIntoSections(this.elementList)
    const innerWidth = this.getInnerWidth()
    const margins = this.getMargins()
    const pageHeight = this.getHeight()
    const mainOuterHeight = this.getMainOuterHeight()
    const startX = margins[3]
    const startY = margins[0] + this.header.getExtraHeight()
    const surroundElementList = [] as IElement[] // v1: ignore surround across columns
    const pageRowList: IRow[][] = []
    let sectionCounter = 0
    for (const sec of sections) {
      const secId = sec.id || `section-${sectionCounter++}`
      const { columnWidth, columnStartXList, columns } =
        this._computeSectionColumns(
          innerWidth,
          sec.config.columns,
          sec.config.gap
        )
      // compute rows constrained by column width
      const rows = this.computeRowList({
        startX,
        startY,
        pageHeight,
        mainOuterHeight,
        isPagingMode: false, // paginate after wrapping
        innerWidth: columnWidth,
        surroundElementList,
        elementList: sec.elements
      })
      // Adjust row.startIndex to map into main elementList indices
      for (let r = 0; r < rows.length; r++) {
        rows[r].startIndex += sec.baseIndex
      }
      // Determine initial Y on the current last page using visual baseline logic
      // Track per-page baseline and per-column offsets similar to Position.computePositionList
      let initialY = 0
      if (pageRowList.length && sec.breakType === 'continuous') {
        const lastPage = pageRowList[pageRowList.length - 1]
        if (lastPage.length) {
          let runStart = 0
          let baseY = 0
          const yByCol = new Map<string, number>()
          const flushRun = (runEnd: number) => {
            if (runEnd <= runStart) return
            const run = lastPage.slice(runStart, runEnd)
            const first = run[0]
            const isColumnRun = !!first.columnInnerWidth && !!first.sectionId
            let consumedY = 0
            for (let k = 0; k < run.length; k++) {
              const rr = run[k]
              consumedY += rr.height + (rr.offsetY || 0)
            }
            if (isColumnRun) {
              const colKey = `${first.sectionId || 'sec'}:${first.columnStartX || 0}:${first.columnInnerWidth || 0}`
              yByCol.set(colKey, (yByCol.get(colKey) || 0) + consumedY)
            } else {
              // finalize columns before accumulating normal run
              if (yByCol.size) {
                let maxY = baseY
                for (const val of yByCol.values()) {
          ...
          for (let r = 1; r < lastPage.length; r++) {
            const prev = lastPage[r - 1]
            const cur = lastPage[r]
            const prevKeyX = prev.columnStartX || 0
            const curKeyX = cur.columnStartX || 0
            const prevW = prev.columnInnerWidth || 0
            const curW = cur.columnInnerWidth || 0
            const sectionChanged =
              (prev.sectionId || '') !== (cur.sectionId || '')
            if (prevKeyX !== curKeyX || prevW !== curW || sectionChanged) {
              flushRun(r)
              // If switching away from a column block, finalize to tallest column
              const runFirst = lastPage[runStart]
              const prevRunIsColumn =
                !!runFirst.columnInnerWidth && !!runFirst.sectionId
              const nextRunIsColumn = !!cur.columnInnerWidth && !!cur.sectionId
              if (prevRunIsColumn && (sectionChanged || !nextRunIsColumn)) {
                if (yByCol.size) {
                  let maxY = baseY
                  for (const val of yByCol.values()) {
                    if (val > maxY) maxY = val
      ...
      // Pre-compute first-page cap height only for multi-column continuous sections
      let firstPageCapHeight: number | undefined
      if (sec.breakType === 'continuous' && columns > 1) {
        let totalRowsHeight = 0
        for (let r = 0; r < rows.length; r++) {
          totalRowsHeight += rows[r].height + (rows[r].offsetY || 0)
        }
        const headerExtraHeight = this.header.getExtraHeight()
        const footerExtraHeight = this.footer.getExtraHeight()
        const pageTopY = margins[0] + headerExtraHeight
        const pageBottomY = this.getHeight() - (margins[2] + footerExtraHeight)
        const pageContentHeight = pageBottomY - pageTopY
        const availableHeightOnPage = Math.max(0, pageContentHeight - initialY)
        const firstRowHeight = rows[0]
          ? rows[0].height + (rows[0].offsetY || 0)
          : 0
        // Cap is the lesser of available height and estimated per-column height,
        // but ensure it's at least one row tall if space permits
        const estimatedPerColumn = Math.ceil(totalRowsHeight / columns)
        const baseCap = Math.min(availableHeightOnPage, estimatedPerColumn)
        firstPageCapHeight = Math.min(
          availableHeightOnPage,
          Math.max(baseCap, firstRowHeight)
        )
      }
      const distributed = this._distributeRowsIntoColumns(
        rows,
        { columnWidth, columnStartXList, columns },
        sec.breakType,
        secId,
        initialY,
        firstPageCapHeight
      )
      // Append to global pages, conditionally merging first distributed page into the existing last page
      if (!pageRowList.length) {
        pageRowList.push(...distributed)
      } else if (sec.breakType === 'continuous' && distributed.length) {
        // Only merge into the existing page if there is room for at least the first row on this page
        const _headerExtraHeight = this.header.getExtraHeight()
        const _footerExtraHeight = this.footer.getExtraHeight()
        const _pageTopY = margins[0] + _headerExtraHeight
        const _pageBottomY =
          this.getHeight() - (margins[2] + _footerExtraHeight)
        const _pageContentHeight = _pageBottomY - _pageTopY
        const availableHeight = Math.max(0, _pageContentHeight - initialY)
        if (initialY > 0 && distributed[0].length) {
          // Merge only rows that fit within available height per column
          const lastPage = pageRowList[pageRowList.length - 1]
          const usedByCol = new Map<number, number>()
          const fitRows: IRow[] = []
          const overflowRows: IRow[] = []
          for (let r = 0; r < distributed[0].length; r++) {
            const row = distributed[0][r]
            const rh = row.height + (row.offsetY || 0)
            const col = row.columnIndex || 0
            const used = usedByCol.get(col) || 0
            if (used + rh <= availableHeight) {
              fitRows.push(row)
              usedByCol.set(col, used + rh)
            } else {
              overflowRows.push(row)
            }
          }
          if (fitRows.length) {
            lastPage.push(...fitRows)
          }
          // Prepare remaining pages: if there are overflow rows, they become the first page; then append the rest
          const remaining = [] as IRow[][]
          if (overflowRows.length) {
            remaining.push(overflowRows)
          }
          if (distributed.length > 1) {
            remaining.push(...distributed.slice(1))
          }
          if (remaining.length) {
            pageRowList.push(...remaining)
          }
        } else {
          // No space on current page for the first row; start on the next page(s)
          pageRowList.push(...distributed)
        }
      } else {
        pageRowList.push(...distributed)
    ...
    // Fallback: if no sections produced pages (empty doc), keep one empty page
    return pageRowList.length ? pageRowList : [[]]
  }

  // Set print data snapshot used in print mode
  public setPrintData() {
    this.printModeData = {
    ...
    // Filter out control assistive elements
  ...
  // Restore original (non-print) data
  ...
  public insertSectionBreak(
    ...
    const element = {
      ...
      sectionId: getUUID(),
      config: {
        columns: Math.max(1, Math.floor(payload.columns)),
        gap: payload.gap ?? defaultGap,
        rule: payload.rule,
        balance: payload.balance
      },
      breakType: payload.breakType || 'continuous'
    } as any as IElement
    this.insertElementList([element], options)
  }
  ...
  // legacy _computePageList removed in favor of section-aware pagination
  ...
  public render(payload?: IDrawOption) {
    ...
    if (isCompute) {
      ...
      // Section-aware 页面信息
      if (isPagingMode) {
        this.pageRowList = this._computePageListWithSections()
        this.rowList = this.pageRowList.flat()
      } else {
        // 连续模式：保留原先逻辑（单流，无分页）
        const margins = this.getMargins()
        const pageHeight = this.getHeight()
